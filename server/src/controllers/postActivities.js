...
const postActivities = async (req, res) => { 
  try { 
      ...
      const existingActivity = await Activity.findAll({
        //para validar q no se repita nombre de actividad con un pais 
          where:{name}, 
          include:{ 
              model:Country, 
              where:{ 
      ...
      if(existingActivity.length>0) {
         // si existen relaciones de nombre de actividad con id de paises          
